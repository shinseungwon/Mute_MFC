...
Mute.h
    ...
    헤더(Resource.h 포함)가 들어 있고 CMuteApp 응용 프로그램
    클래스를 선언합니다.

Mute.cpp
    응용 프로그램 클래스 CMuteApp이(가) 들어 있는 기본 응용 프로그램
    소스 파일입니다.
...
응용 프로그램 마법사에서 하나의 뷰가 만들어집니다.

MuteView.h, MuteView.cpp - 문서 뷰
    이 파일에는 CMuteView 클래스가 들어 있습니다.
...
MuteSet.h, MuteSet.cpp
    이 파일에는 CMuteSet 클래스가 들어 있습니다. 이 클래스는 마법사에서 선택한
    데이터 소스에 액세스하는 데 사용됩니다.
    serialization 지원은 추가되지 않습니다.
...
StdAfx.h, StdAfx.cpp
    이 파일은 미리 컴파일된 헤더 파일(PCH)인 Mute.pch와
